...
def dijkstra(graph, start):
    ...
    #add start node to the confirmed dictionary 
    confirmed[start] = 0 

    #place the start nodes neighbors in the tentatitve dictionary 
    ...
    while tentative: 
        ...
        #select node from the tentative list with the lowest cost 
        for node, distance in tentative.items():
            #determine shnortest distance node
        ...
        #remove next ndoe from tentative dictionary
        tentative.pop(next)

        #place next nodes neighbors on tentative dictionary 
        for node, distance in graph[next].items():
            cost = confirmed[next] + distance 
            #if the neighbor is not in tentative , add to tentative 
            #if neighbor in tentative, update cost if neccessary
            #if neighbor is confirmed then ignore 
